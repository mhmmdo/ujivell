...
@section('content')
    <div class="page-header">
        <div>
            <h1 class="page-title">Edit Data Peserta</h1>
            <p class="page-subtitle">Perbarui informasi data peserta sertifikasi yang sudah ada.</p>
        </div>
    ...
    <div class="card form-card">
        <form action="{{ route('peserta.update', $peserta) }}" method="POST" class="custom-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input
                        ...
                        placeholder="Contoh: PSR-2026-001"
                        autofocus
                    >
                    @error('nomor_peserta')
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                        ...
                        <option value="Terdaftar" {{ old('status', $peserta->status) == 'Terdaftar' ? 'selected' : '' }}>
                            Terdaftar
                        </option>
                        <option value="Proses" {{ old('status', $peserta->status) == 'Proses' ? 'selected' : '' }}>
                            Proses
                        </option>
                        <option value="Kompeten" {{ old('status', $peserta->status) == 'Kompeten' ? 'selected' : '' }}>
                            Kompeten
                        </option>
                        <option value="Belum Kompeten" {{ old('status', $peserta->status) == 'Belum Kompeten' ? 'selected' : '' }}>
                            Belum Kompeten
                        </option>
                    </select>
                    @error('status')
                ...
                <div class="form-group">
                    ...
                    <input
                        ...
                        value="{{ old('tanggal_uji', $peserta->tanggal_uji) }}"
                    >
                    @error('tanggal_uji')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
